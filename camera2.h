...
public:
	...
	Camera2(float posX, float posZ, float upX, float upZ, float yaw, float pitch) : Front(glm::vec3(0.0f, 0.0f, -1.0f)), MovementSpeed(SPEED), MouseSensitivity(SENSITIVITY), Zoom(ZOOM)
	{
		Position = glm::vec3(posX, posZ);
		WorldUp = glm::vec3(upX, upZ);
		Yaw = yaw;
		Pitch = pitch;
		updateCameraVectors();
	}
